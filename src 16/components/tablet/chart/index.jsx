...
const DailyTransactionChart = () => {
  ...
  return (
    <Stack
      ...
      <Stack horizontal={!isCompact} gap={12} justifyContent="space-between" alignItems={isCompact ? 'flex-start' : 'center'}>
        <Stack horizontal gap={8} flexWrap="wrap" alignItems="center">
          {PERIODS.map(p => (
            <PeriodChip
              key={p.key}
              label={p.label}
