...
const queryMenuByNamePrefix = async (searchString: string): Promise<Menu[]> => {
  try {
    const realm = await getRealmInstance();
    const nameFilter = 'name BEGINSWITH[c] $0';
    const cleanedSearchString = searchString ? searchString.trim() : '';
    ...
    const menusQuery = realm
      .objects<Menu>('Menu')
      .filtered(nameFilter, cleanedSearchString);

    const menusWithAddOns = await Promise.all(
      menusQuery.map(async (menu) => {
        const menuObject = {
          menu_id: menu.menu_id,
          name: menu.name,
          bar_code: menu.bar_code,
    ...
    console.error("Error querying menu by name prefix:", error);
    throw error;
  }
};
...
const getMenuIconById = async (menu_id: string): Promise<{menu_id: string; name: string; icon_name: string | undefined} | null> => {
  try {
    const realm = await getRealmInstance();
