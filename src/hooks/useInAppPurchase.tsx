...
const useProductManager = () => {
  ...
  const fetchProduct = useCallback(async () => {
    if (connected && products.length === 0) {
      try {
        await getProducts({ skus: [PRODUCT_SKU] });
  ...
  }, [connected, products, getProducts]);
...
const usePurchaseErrorHandler = (setData: Dispatch<SetStateAction<PurchaseData>>) => {
  ...
  const handlePurchaseError = useCallback((error: PurchaseError | null) => {
    ...
    switch (error.code) {
      ...
      case 'E_NETWORK_ERROR':
        ...
        break;
      default:
        errorMessage = `Purchase failed: ${error.message || 'Unknown error'}`;
    }
  ...
  useEffect(() => {
    handlePurchaseError(currentPurchaseError ?? null);
...
const useInAppPurchase = () => {
  ...
  const { product, fetchProduct } = useProductManager();
  ...
  }, [isInitialized, initializePurchaseStatus, fetchProduct]);
