...
import { useUpdateAddOn } from "../../../hooks/useAddon";
...
const EditAddOn = () => {
  ...
  const { update, error, loading, resetHandler } = useUpdateAddOn()
  const { addOn, menu } = route.params
  ...
  const onSubmit = async () => {
    ...
    await update(fields.addOn_id, fields.addOnName, parseFloat(fields.price), fields.status).then(async (result) => {
      result && (
        navigator.reset({
          key: 'addOns',
          index: 0,
    ...
    <StyledSafeAreaView backgroundColor={theme.colors.gray[1]}>
      <StyledHeader marginHorizontal={8} statusProps={{ translucent: true }} >
        <StyledHeader.Header onPress={() => navigator.reset({
          ...
          index: 0,
          routes: [{ name: 'addOns', params: { menu: menu } }]
        })} title='Edit AddOn' icon cycleProps={{
